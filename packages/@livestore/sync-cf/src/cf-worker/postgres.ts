...
export type PostgresMakeDurableObjectClass = (options?: PostgresMakeDurableObjectClassOptions) => {
            ...
            case 'WSMessage.PullReq': {
              ...
              const batches =
                remainingEvents.length === 0
                  ? [[]]
                  : Array.from({ length: Math.ceil(remainingEvents.length / POSTGRES_CHUNK_SIZE) }, (_, i) =>
                      remainingEvents.slice(i * POSTGRES_CHUNK_SIZE, (i + 1) * POSTGRES_CHUNK_SIZE),
                    )

              console.log('🐘🔌 after batches')

              for (const [index, batch] of batches.entries()) {
                const remaining = Math.max(0, remainingEvents.length - (index + 1) * POSTGRES_CHUNK_SIZE)
...
const makeStorage = (ctx: DurableObjectState, env: PostgresEnv, storeId: string, pgClient: Client): SyncStorage => {
